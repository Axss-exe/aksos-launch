...
const mockRitaStories = [
  { label: 'STORY 01 / MINING + REGULATION', title: 'A regulation is rarely only a regulation.', body: 'RITA can connect a new policy signal to the companies, institutions, locations and earlier decisions that give it meaning.', links: ['New regulation', 'Mining company', 'Government', 'Investment'] },
  { label: 'STORY 02 / INFRASTRUCTURE + CAPITAL', title: 'A project announcement can reveal a larger movement.', body: 'The announcement becomes more useful when it is placed beside funding, procurement, implementation partners and the places affected.', links: ['Project signal', 'Funding body', 'Implementing partner', 'Location'] },
...
function LegacyRITARedesign() {
  ...
  const nodes = [
    ...
    ['Previous policy', 'node-policy'], ['Investment', 'node-investment'], ['Person', 'node-person'],
    ['Market', 'node-market'], ['Location', 'node-location'],
  ]
  return <section className="section rita-redesign" id="rita-redesign"><div className="rita-redesign-head"><SectionLabel>06 / THE RELATIONSHIP LAYER</SectionLabel><h2>Not every connection<br /><em>is a story.</em></h2><p>RITA reveals which relationships around a new event are meaningful enough to change what it means.</p></div><div className="rita-visual" role="img" aria-label="An isolated mining regulation becomes a connected intelligence picture through RITA"><div className="rita-before"><span>BEFORE RITA</span><b>A new mining regulation<br />is announced.</b><small>One event. Limited context.</small></div><div className="network-canvas"><div className="network-lines" aria-hidden="true"><i className="line line-government" /><i className="line line-regulation" /><i className="line line-company" /><i className="line line-policy" /><i className="line line-investment" /><i className="line line-person" /><i className="line line-market" /><i className="line line-location" /></div><div className="event-node"><span>NEW INFORMATION</span><b>Mining regulation<br />announced</b></div>{nodes.map(([label, className]) => <span className={`knowledge-node ${className}`} key={label}>{label}</span>)}<div className="rita-lens"><img src="/rita-symbol-traced.svg" alt="" /><img src="/rita-wordmark.svg" alt="RITA" /><small>reveals the<br />relationships that matter</small></div><div className="network-legend"><span><i className="legend-core" />Core relationship</span><span><i className="legend-context" />Contextual relationship</span></div></div><div className="rita-after"><span>AFTER RITA</span><b>One event, now connected<br />to a larger picture.</b><div className="meaning-list"><span>Relationships</span><span>Context</span><span>Implications</span><span>Possible opportunities / risks</span></div></div></div><p className="rita-principle">Information tells you what happened.<br /><em>Relationships reveal why it matters.</em></p><div className="rita-stories"><div className="stories-header"><div><span className="type-number">FIELD NOTES / REPRESENTATIVE STORIES</span><h3>One signal.<br /><em>A larger picture.</em></h3></div><div className="story-controls"><button type="button" onClick={() => setStoryIndex((storyIndex + mockRitaStories.length - 1) % mockRitaStories.length)} aria-label="Previous story">←</button><span>0{storyIndex + 1} / 0{mockRitaStories.length}</span><button type="button" onClick={() => setStoryIndex((storyIndex + 1) % mockRitaStories.length)} aria-label="Next story">→</button></div></div><article className="story-card"><div><span className="type-number">{mockRitaStories[storyIndex].label}</span><h4>{mockRitaStories[storyIndex].title}</h4><p>{mockRitaStories[storyIndex].body}</p></div><div className="story-links" aria-label="Connected entities">{mockRitaStories[storyIndex].links.map((link, i) => <span key={link}><b>0{i + 1}</b>{link}{i < mockRitaStories[storyIndex].links.length - 1 && <ChevronRight />}</span>)}</div></article></div></section>
}

function AtisValidationSection() {
  const validationSteps = [
    ['01', 'Collect', 'Bring together documents, signals, and first-source context.'],
    ['02', 'Check', 'Compare sources, dates, provenance, and the strength of the evidence.'],
    ['03', 'Connect', 'Place each fact inside the relationships that give it meaning.'],
    ['04', 'Explain', 'Return a traceable picture with uncertainty, implications, and next steps.'],
  ]
  return <section className="atis-validation" aria-labelledby="atis-validation-title"><div className="atis-validation-head"><SectionLabel>04A / HOW ATIS VALIDATES</SectionLabel><h2 id="atis-validation-title">Before an answer becomes useful,<br /><em>the evidence has to hold.</em></h2><p>ATIS is being built to do more than retrieve information. It validates what it finds, shows how the pieces connect, and makes the path back to the evidence visible.</p><a className="button button-quiet" href="/batana">Learn more about the system <ArrowUpRight /></a></div><div className="atis-validation-grid">{validationSteps.map(([number, title, copy]) => <div key={number} className="atis-validation-step"><span>{number}</span><h3>{title}</h3><p>{copy}</p></div>)}</div><div className="atis-validation-note"><span className="query-dot" />The output is not just an answer. It is a clearer basis for deciding what to do next.</div></section>
}
...
export function AksosSite() {
  ...
  return (
    <main>
      ...
      <section className="section atis" id="atis"><div className="atis-header"><div><SectionLabel>04 / THE SYSTEM</SectionLabel><div className="atis-title"><Mark atis /><h2>ATIS</h2></div><p className="subhead">An intelligence platform we&apos;re building to connect evidence, relationships and emerging information.</p></div><a className="atis-arrow" href="https://atis.aksos.net" target="_blank" rel="noreferrer" aria-label="Open the ATIS pilot briefing"><ArrowUpRight /></a></div><div className="atis-content"><div><p>ATIS is an intelligence platform powered by a Retrieval-Augmented Generation system and built around verified knowledge.</p><p>It is being developed to work across economic, legal, regulatory, government, organizational and other relevant information.</p><p>Users can ask questions in natural language and explore the evidence, relationships and context surrounding a subject. The system can then produce analytical reports that bring together relevant evidence, relationships, risks, opportunities and strategic considerations.</p><p className="fine-print">ATIS is still under active development. What you see here represents capabilities demonstrated by the current system — not the finished vision.</p></div><DemoPanel /></div></section><AtisValidationSection />

      <section className="section value-flow"><SectionLabel>05 / HOW INFORMATION BECOMES INTELLIGENCE</SectionLabel><div className="cap-header"><h2>Better signals.<br /><em>Clearer understanding.</em></h2><p>ATIS is being built to add value between the moment information appears and the moment someone needs to act on what it may mean.</p></div><InformationFlow /></section><section className="section capabilities"><SectionLabel>06 / CURRENT CAPABILITIES</SectionLabel><div className="cap-header"><h2>What ATIS can do today.</h2><p>Here is what we have been able to build so far. The interface is a window into an evolving system, not a promise of a finished product.</p></div><div className="cap-list"><div><span>01</span><b>Natural-language querying</b><p>Ask questions across a verified knowledge base.</p></div><div><span>02</span><b>Evidence retrieval</b><p>Trace an answer back to the sources that support it.</p></div><div><span>03</span><b>Relationship identification</b><p>Surface how entities, events and documents may connect.</p></div><div><span>04</span><b>Analytical reporting</b><p>Bring evidence, context and potential implications together.</p></div></div></section>

      <RITARedesign />
      <section className="section rita" id="rita"><div className="rita-heading"><SectionLabel>06 / THE RELATIONSHIP LAYER</SectionLabel><div className="rita-title"><div className="rita-identity"><img className="rita-symbol" src="/rita-symbol-traced.svg" alt="" /><img className="rita-wordmark" src="/rita-wordmark.svg" alt="RITA" /></div><h2>Not every connection<br /><em>is a story.</em></h2></div></div><div className="rita-copy"><p className="subhead">RITA — Relationship Intelligence &amp; Triage Analyst</p><div><p>A new event is only the beginning. RITA connects it to the wider ATIS knowledge base so the relationships, implications and possible opportunities around it become visible.</p><p>It is being developed to distinguish meaningful relationships from generic contextual overlap.</p></div></div><div className="rita-value-diagram"><div className="rita-stage input-stage"><span className="type-number">01 / NEW INFORMATION</span><strong>What happened?</strong><small>Event · document · signal</small></div><div className="rita-connector" /><div className="rita-stage knowledge-stage"><span className="type-number">02 / ATIS KNOWLEDGE BASE</span><strong>What does ATIS already know?</strong><div className="knowledge-points"><span>Entities</span><span>Relationships</span><span>Historical context</span><span>Government</span><span>Companies</span><span>Markets</span><span>Regulations</span><span>People</span><span>Previous events</span></div></div><div className="rita-connector" /><div className="rita-stage rita-stage-main"><span className="type-number">03 / RITA</span><strong>What connects?</strong><div className="stage-list"><span>Connects relevant information</span><span>Adds context and provenance</span><span>Finds cross-border implications</span><span>Detects strategic connections</span></div></div><div className="rita-connector" /><div className="rita-stage output-stage"><span className="type-number">04 / NEW INTELLIGENCE</span><strong>What could this mean?</strong><div className="stage-list"><span>Who is affected</span><span>What changed</span><span>Potential opportunities and risks</span><span>What actions become possible</span></div></div></div><div className="relationship-types"><div><span className="type-number">01</span><h3>Semantic relationships</h3><p>Instead of simply recording that two entities appear in the same document, RITA attempts to understand the relationship between them.</p><div className="relationship-examples"><span>Program <b>→ implemented by →</b> Organization</span><span>Policy <b>→ affects →</b> Industry</span><span>Organization <b>→ responsible for →</b> Initiative</span></div></div><div><span className="type-number">02</span><h3>Relationship triage</h3><p>Not every relationship deserves equal analytical weight. RITA evaluates relationship type, semantic strength, evidence quality, source support, recurrence, narrative coherence and provenance.</p><div className="triage-bars"><span style={{'--width':'92%'} as React.CSSProperties}>source support</span><span style={{'--width':'76%'} as React.CSSProperties}>coherence</span><span style={{'--width':'58%'} as React.CSSProperties}>recurrence</span></div></div></div><div className="graphs"><div><b>STORY GRAPH</b><span>Meaningful relationships that can contribute to a coherent narrative.</span></div><div><b>CONTEXT GRAPH</b><span>Useful relationships that provide context but are not strong enough to establish a story.</span></div></div><p className="rita-end">RITA&apos;s job is not to find the maximum number of connections.<br /><em>Its job is to find the right connections.</em></p></section>
      ...
      <section className="section network-section" id="connect"><div className="network-copy"><SectionLabel>10 / THE NETWORK</SectionLabel><h2>We&apos;re building<br /><em>the network, too.</em></h2><p>Some of the information that matters most isn&apos;t sitting neatly on the internet. It lives inside institutions, organizations, businesses, research communities and the people working within them.</p><p>We are currently talking to people across different fields and looking for relationships that can help us understand the problem better and build the system properly.</p><a className="button button-solid" href="mailto:connect@aksos.net">Start a conversation <Mail /></a></div><div className="network-orbit"><div className="orbit-center">AKSOS</div><span className="orbit-item oi-1">RESEARCHERS</span><span className="orbit-item oi-2">INSTITUTIONS</span><span className="orbit-item oi-3">BUILDERS</span><span className="orbit-item oi-4">EXPERTS</span><span className="orbit-item oi-5">INFORMATION</span></div></section>
