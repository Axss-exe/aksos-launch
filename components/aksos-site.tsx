...
function RITARedesign() {
  const knowledgeNodes = ['Company', 'Policy', 'Institution', 'Person', 'Market', 'Event']
  const storyNodes = ['New signal', 'Policy', 'Institution', 'Market']
  const contextNodes = ['Company', 'Person', 'Previous event', 'Regulation']
  return <section className="section rita-redesign" id="rita-redesign"><div className="rita-redesign-head"><SectionLabel>06 / THE RELATIONSHIP LAYER</SectionLabel><h2>Not every connection<br /><em>is a story.</em></h2><p>RITA connects new information to the wider ATIS knowledge base so relationships, implications and possible opportunities become visible.</p></div><div className="rita-system"><div className="rita-input"><span className="type-number">01 / NEW INFORMATION</span><strong>What happened?</strong><small>Event · document · signal</small></div><div className="rita-arrow-line" /><div className="rita-knowledge"><div className="rita-stage-head"><span className="type-number">02 / ATIS KNOWLEDGE</span><strong>What does ATIS already know?</strong></div><div className="knowledge-network" aria-label="Connected ATIS knowledge network">{knowledgeNodes.map((node, i) => <span className={`network-node network-node-${i + 1}`} key={node}>{node}</span>)}<i className="network-line network-line-1" /><i className="network-line network-line-2" /><i className="network-line network-line-3" /><i className="network-line network-line-4" /><i className="network-line network-line-5" /><b className="network-core">ATIS<br /><em>knowledge</em></b></div></div><div className="rita-arrow-line" /><div className="rita-core"><div className="rita-core-brand"><img src="/rita-symbol-traced.svg" alt="" /><img src="/rita-wordmark.svg" alt="RITA" /></div><span className="type-number">03 / RELATIONSHIP INTELLIGENCE + TRIAGE</span><strong>Find the relationships<br />that matter.</strong><div className="rita-core-list"><span>Shared entities + organisations</span><span>Events, places + legislation</span><span>Temporal + causal signals</span><span>Semantic overlap</span></div></div><div className="rita-arrow-line" /><div className="rita-output"><span className="type-number">04 / THE RESULT</span><strong>Story or context?</strong><div className="graph-split"><div><b>STORY GRAPH</b><small>Meaningful relationships that contribute to a coherent narrative.</small><div className="mini-graph story-mini">{storyNodes.map((node, i) => <span key={node} className={`mini-node mini-node-${i + 1}`}>{node}</span>)}</div></div><div><b>CONTEXT GRAPH</b><small>Useful relationships that provide context without establishing a story.</small><div className="mini-graph context-mini">{contextNodes.map((node, i) => <span key={node} className={`mini-node mini-node-${i + 1}`}>{node}</span>)}</div></div></div></div></div><div className="rita-intelligence"><span className="type-number">05 / EMERGING INTELLIGENCE</span><strong>New intelligence from existing knowledge.</strong><div><span>Clusters + hidden paths</span><span>Bridge nodes</span><span>Contradictions</span><span>Narrative patterns</span></div></div><p className="rita-principle">RITA is not trying to find the maximum number of connections.<br /><em>Its job is to find the right connections.</em></p></section>
}
