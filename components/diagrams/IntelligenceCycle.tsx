...
// Inner ring nodes (relationship intelligence process)
const INNER_RING_NODES = [
  { label: 'IDENTIFY', delay: 0.1 },
  { label: 'ANALYZE', delay: 0.15 },
  { label: 'CONNECT', delay: 0.2 },
  { label: 'VERIFY', delay: 0.25 },
];
...
export function IntelligenceCycle() {
  ...
  return (
    <motion.div
      className="intelligence-cycle-diagram"
      initial={{ opacity: 0 }}
    ...
    >
      ...
      >
        ...
        {layout.innerRing.map((innerNode, innerIndex) => {
          ...
          // Find closest outer node for connection
          // In desktop: each inner node connects to outer nodes in its quadrant
          // Simplified: connect each inner node to next outer node
        ...
        {isHovered && breakpoint !== 'mobile' && (
          <motion.text
            ...
            animate={{ opacity: 1 }}
          >
            HOVER TO EXPLORE RELATIONSHIP INTELLIGENCE
          </motion.text>
        )}
